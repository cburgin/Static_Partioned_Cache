...
class cache:
    ...
    def __init__(self, cacheSize, blockSize, mapping):
        ...
        self.numSets = self.cacheSize / (self.cacheMap * self.blockSize)
    ...
    def simulate_element(element):
        ...
        curr_set = (address / self.blockSize) % self.numSets
        curr_tag = (address / self.blockSize) / self.numSets

        #Increment every location in the LRU every time there is a cache access
        for i in range(len(self.numSets)):
            for j in range(len(self.cacheMap)):
                self.lru[i][j] += 1

        if read_write == 'read':
            found = False
            for i in range(len(self.cacheMap)):
                if self.cache[curr_set][i]['tag'] == curr_tag && self.cache[curr_set][i]['valid'] != 0:
            ...
            if !found:
                ...
                for j in range(len(self.cacheMap)):
                    if self.cache[curr_set][j]['valid'] == 0 && !found:
                ...
                if !found:
                    ...
                    lru[curr_set][index] = 0

                #Update the cache information
                self.myCache[curr_set][index]['tag'] = curr_tag
                self.myCache[curr_set][index]['valid'] = 1

        elif read_write == 'write':
            found = False
            index = 0
            for i in range(len(self.cacheMap)):
                if self.cache[curr_set][i]['tag'] == curr_tag && !found:
            ...
            if !found:
                for i in range(len(self.cacheMap)):
                    if self.cache[curr_set][i]['valid'] == 0 && !found:
                        #Loop until and invalid location is found.  Once found
                        #set found to true, reset the lru and mark the index that
                        #it was found in.
                        found = True
